  ...
  addLayers_(root) {
    ...
    while (fifo.length > 0) {
      ...
      if (olLayer instanceof olLayerGroup) {
        ...
        if (olLayerWithParents.layer.values_.id == 'xzqNavigateLayer' ) {
          cesiumObjects = this.createSingleLayerCounterparts(olLayerWithParents);
          myLayerInfo = olLayerWithParents.layer.values_.info;
          if (!cesiumObjects) {
            ...
            const onLayerChange = (e) => {
              ...
              if (cesiumObjs) {
                ...
                if (myLayerInfo && myLayerInfo.hasOwnProperty('isBaseMapLayer') && myLayerInfo.isBaseMapLayer == 1) {
                  console.log('图层来自baseMap');
                } else {
                  this.addCesiumObjects_(cesiumObjs, layerId, layerWithParents.layer);
                }
                this.orderLayers();
      ...
      if (collection) {
        contentKeys = [
          collection.on('add', (event) => {
            if (!event.element.values_.hasOwnProperty('info')) {
              this.addLayers_(event.element);
            } else if (!event.element.values_.info.hasOwnProperty('isBaseMapLayer') || !event.element.values_.info.isBaseMapLayer) {
              this.addLayers_(event.element);
            }
            // this.addLayers_(event.element);
          }),
