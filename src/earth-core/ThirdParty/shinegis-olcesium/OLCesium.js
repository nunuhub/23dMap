...
class OLCesium {
     ...
     */
    constructor(options) {
        this.isBaseMap=options.isBaseMap?options.isBaseMap:false
        /**
        ...
        if (this.scene_) {
            ...
            //this.container_ = document.getElementsByClassName("cesium-widget")
            this.container_ = document.querySelector(".cesium-widget");
            ...
            //let a = this.canvas_.style+"visibility:hidden"
             ...
             * @private
             */
            this.container_ = document.createElement('DIV666');
        ...
        if(this.isBaseMap==false){
            this.synchronizers = options.createSynchronizers ?
                options.createSynchronizers(this.map_, this.scene_, this.dataSourceCollection_) : [
                    new olcsRasterSynchronizer(this.map_, this.scene_),
                    new olcsVectorSynchronizer(this.map_, this.scene_),
                    new olcsOverlaySynchronizer(this.map_, this.scene_)
            ...
            for (let i = this.synchronizers.length - 1; i >= 0; --i) {
                this.synchronizers[i].synchronize();
            }
        ...
        eventHelper.add(this.scene_.preRender, OLCesium.prototype.updateTrackedEntity_, this);
    ...
    update(isSplit) {
        // Assures correct canvas size after initialisation
        if(isSplit){
            this.map_.isSplit = isSplit; // 分屏用
        }
        this.synchronizers = this.options_.createSynchronizers ?
            this.options_.createSynchronizers(this.map_, this.scene_, this.dataSourceCollection_) : [
               new olcsRasterSynchronizer(this.map_, this.scene_),
               new olcsVectorSynchronizer(this.map_, this.scene_),
               new olcsOverlaySynchronizer(this.map_, this.scene_)
            ];
    ...
    setEnabled(enable, isSplit) {
        ...
        if (this.enabled_) {
            ...
            if (this.isOverMap_) {
                ...
                // interactions.clear();
                // console.log("this.map", this.map_.getLayerById('xzqNavigateLayer').getSource().getFeatures());
                // if(this.map_.getLayerById('xzqNavigateLayer')){
                //     debugger
                //     let _xzqNavigateFeatures = this.map_.getLayerById('xzqNavigateLayer').getSource().getFeatures();
                //     let geo = new GeoJSON({
                //       dataProjection: "EPSG:4326",
                //       featureProjection: "EPSG:3857",
                //     }).writeFeatures(_xzqNavigateFeatures);
                //     console.log('geo', JSON.parse(geo));
                //     let _vectorSource = new VectorSource();
                //     _vectorSource.addFeatures(JSON.parse(geo));
                //     console.log('1111', _vectorSource.getFeatures())
                //     this.map_.getLayerById('xzqNavigateLayer').getSource().clear();
                //     // this.map_.getLayerById('xzqNavigateLayer').setSource(_vectorSource)
                //     // this.map_.getLayerById('xzqNavigateLayer').getSource().refresh()
                //     // console.log("this.map1", this.map_.getLayerById('xzqNavigateLayer').getSource().getFeatures());
                //     // setTimeout(() => {
                //     //     console.log("this.map1", this.map_.getLayerById('xzqNavigateLayer').getSource().getFeatures());
                //     // }, 1000);
                // }
                ...
                if (rootGroup.getVisible()) {
                    ...
                    this.hiddenRootGroup_.setVisible(isSplit? true: false);
                }
