...
exports.sourceToImageryProvider = function (olMap, source, viewProj, olLayer) {
  ...
  if (source instanceof olSourceImageWMS && source.getUrl() &&
    ...
    /*source.on('imageloadstart', (img) => {
        console.log('imageloadstart', img)
        imageSize = img.target.imageSize_
        const sourceProps = {
  ...
  if (source instanceof olSourceTileImage /*|| source instanceof olSourceImageWMS*/) {
    ...
    if (exports.isCesiumProjection(projection)) {
     if(source.layer_){
       ...
       let a = source.key_;
       let b=a.match(/LAYERS-(\S*)\/VERSION-1.1.0/)[1];
       ...
       opts.layers = b
       provider = new Cesium.WebMapServiceImageryProvider(opts)
     };
  ...
  } else if (source instanceof olSourceImageStatic /*|| source instanceof olSourceImageWMS*/) {
    let projection = olcsUtil.getSourceProjection(source);
    if (!projection) {
      projection = viewProj;
    }
...
exports.ol4326CoordinateToCesiumCartesian = function (coordinate) {
  const coo = coordinate;
  if (coordinate[0] > 180) {
    let a = new Cesium.WebMercatorProjection()
    let b = coo.length > 2 ? a.unproject({x: coo[0], y: coo[1], z: coo[2]}) : a.unproject({x: coo[0], y: coo[1]})
    let c = Cesium.Cartographic.toCartesian(b)
    return c
  }
  // const coo = coordinate;
  return coo.length > 2 ?
...
exports.isCesiumProjection = function (projection) {
  ...
  const is4490 = projection === getProjection('EPSG:4490')
  return is3857 || is4326 ||is4490;
};
