...
class RoamFly {
  constructor(prop) {
    ...
    this.init();
    this.clock = this.prop.viewer.myClock;
  }
  ...
  realTime(position) {
    ...
    /* const time =
      (this.clock.currentTime.dayNumber - this.clock.startTime.dayNumber) *
        24 *
        60 *
        60 +
      this.clock.currentTime.secondsOfDay -
      this.clock.startTime.secondsOfDay; // 已飞行时间 */
  ...
  _startFly() {
    ...
    let startTime = Cesium.JulianDate.fromIso8601('2013-12-25');
    let stopTime; // 飞行结束时间
    ...
    // this.clock.clockRange = Cesium.ClockRange.UNBOUNDED //达到终止时间后继续读秒
    ...
    switch (attr.point) {
      case 'point':
        ...
        break;
      case 'model_car': // 汽车模型
        // entityAttr.model = { uri: "data3d/gltf/qiche.gltf", scale: 0.2, minimumPixelSize: 50 }

        // entityAttr.model = { uri: require("../Assets/model/car.gltf"), scale: 0.2, minimumPixelSize: 50 }

        entityAttr.model = {
          uri: 'Assets3D/model/car.gltf',
          scale: 0.2,
          minimumPixelSize: 50
        };
        if (entityAttr.label) {
        ...
        break;
      case 'model_air': // 飞机模型
        // entityAttr.model = { uri: "data3d/gltf/feiji.glb", scale: 0.1, minimumPixelSize: 50 }
        // console.log('entityAttrentityAttrentityAttr', entityAttr)
        // entityAttr.model = { uri: require("../Assets/model/plane.glb"), scale: 0.1, minimumPixelSize: 50 }
        entityAttr.model = {
          uri: 'Assets3D/model/plane.glb',
          scale: 0.2,
          minimumPixelSize: 50
        };
        if (entityAttr.label) {
      ...
      case 'model_weixin': // 卫星模型
        entityAttr.model = {
          uri: 'Assets3D/model/satelight.gltf',
          scale: 1,
          minimumPixelSize: 100
        };
        // entityAttr.model = { uri: require("../Assets/model/satelight.gltf"), scale: 1, minimumPixelSize: 100 }
        if (entityAttr.label) {
          entityAttr.label.pixelOffset = new Cesium.Cartesian2(60, -30);
        } // 偏移量
        break;
  ...
  getFlyOkPoints(position) {
    ...
    const thistime = this.clock.currentTime;
  ...
  recordPause() {
    // const { viewer } = this.prop;
    this.recoverTime = this.clock.currentTime;
    // this.isPause = true
    // this.isStart = false
  }
  ...
  createTimeLine() {
    ...
    if (!viewer.animation) {
      ...
      const animation = new Cesium.Animation(
        animationContainer,
        new Cesium.AnimationViewModel(this.clockViewModel)
      );
    ...
    if (!viewer.timeline) {
      ...
      const timeline = new Cesium.Timeline(timelineContainer, this.clock);
      timeline.addEventListener('settime', this.onTimelineScrubfunction, false);
      timeline.zoomTo(this.clock.startTime, this.clock.stopTime);
  ...
  preRender_eventHandler() {
    ...
    if (
      Cesium.JulianDate.compare(this.clock.currentTime, this.clock.stopTime) >=
      0
    )
      return this.onTimeEnd();

    if (this.isPause) {
      this.clock.currentTime = this.recoverTime;
    } else {
      if (typeof this.isBackWard == 'boolean') {
        if (this.isBackWard) {
          if (this.idxOfTimes == 1) {
            // 如果是第一个节点，继续回到第一节点
            this.idxOfTimes = 2;
          }
          this.clock.currentTime = this.timesAtPoint[this.idxOfTimes - 2];
          this.isBackWard = '';
          // this.clock.currentTime.secondsOfDay -= 0.051
        } else {
          this.clock.currentTime = this.timesAtPoint[this.idxOfTimes];
          this.isBackWard = '';
        }
      }
    }
    ...
    switch (attr.cameraType) {
      ...
      /*             if (viewer.trackedEntity != undefined) { viewer.trackedEntity = undefined }
                  ...
                  // const targetPos = this.flyEntity.position.getValue(this.clock.currentTime)
    ...
    // this.clock.currentTime, this.positionScratch)
    // if (position) {
    //   //实时监控
    //   this.realTime(position)
    // }

    /* if (JSON.stringify(this.clock.currentTime) ==
 JSON.stringify(this.clock.stopTime)) {
 this.onTimeEnd()
 //this.clock.currentTime = this.clock.startTime, this.onTimeEnd()
 } */

    /* if (this.isBackWard && JSON.stringify(this.clock.currentTime) <=
     JSON.stringify(this.clock.startTime)) {
