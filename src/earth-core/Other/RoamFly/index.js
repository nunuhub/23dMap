...
import { getAngle, isNumber, formatLength } from "../../Tool/Util1"
import { formatPosition, terrainPolyline } from "../../Tool/Point2"
...
class RoamFly {
    ...
    /**
     * 构造一个飞行
     * @param prop {object} 飞行配置
     */
    constructor(prop) {
        const { viewer, route } = prop || {}

        /**
         * @enum {object} prop
         */
        this.prop = {
            /** @type {Cesium.Viewer} Cesium.Viewer的一个实例 */
            viewer,
            /** @type {object} 飞行路线的json对象 */
            route
        }
    ...
    init() {
        ...
        this.firstHpr = this.setOrientation(Cesium.Cartesian3.fromDegrees(...hFirstP), Cesium.Cartesian3.fromDegrees(...secondP))
        this.firstHpr.pitch = -Math.atan2(height, height * 2)
        viewer.camera.flyTo({ destination: Cesium.Cartesian3.fromDegrees(...hFirstP), orientation: this.firstHpr })
    ...
    /**
     * 开始飞行
     * @param route {object} 路线json，如果不传，那么将使用上一次设置的route进行飞行。
     */
    start(route) {
    ...
    stop() {
        ...
        viewer.scene.preRender.removeEventListener(this.preRender_eventHandler, this)
    ...
    /**
     * 显示当前时间
     * @param position {Cesium.Cartesian} 位置
     */
    realTime(position) {
        const { viewer } = this.prop
        ...
        const time = (viewer.clock.currentTime.dayNumber - viewer.clock.startTime.dayNumber) *
            24 * 60 * 60 +
            viewer.clock.currentTime.secondsOfDay - viewer.clock.startTime.secondsOfDay// 已飞行时间
    ...
    getTerrainHeight(calback) {
        ...
        let alllen = 0, arrLength = [], arrHbgd = [], arrFxgd = [], arrPoint = [], arrBjgd
        if (attr.showHeightWarn && attr.warnHeight) {
            arrBjgd = []
        }
    ...
    _startFly() {
        ...
        for (let i = 0, length = lonlats.length; i < length; i++) {
            ...
            if (i === 0) { // 起点
                ...
                let speed = isSpeedArray ? (speeds ? speeds[i - 1] : defSpeed) : (speeds || defSpeed)
                ...
                let speed = isSpeedArray ? (speeds ? speeds[i - 1] : defSpeed) : (speeds || defSpeed) // 1千米/时 =  1/3.6 米/秒
                // speed = speed * 0.8//转弯处降速

                let arrBezier = this.getBezierSpline(lonlats[i - 1], lonlat, lonlats[i + 1])
        ...
        const allInfo = { name: attr.name, alllen: alllen * 1000, alltime: alltimes }
        console.log(allInfo)
        ...
        if (attr.showLine) { // 是否显示路线
            entityAttr.path = {
                material: new Cesium.Color.fromCssColorString(route.properties.style.color).withAlpha(0.4),
                resolution: 1, width: 1.5, leadTime: 0, trailTime: alltimes
            }
        }
        // 漫游对象
        switch (attr.point) {
            case "point":
                entityAttr.point = {
                    color: new Cesium.Color.fromCssColorString(route.properties.style.color).withAlpha(0.8),
                    pixelSize: 6
                }
                if (entityAttr.label) {
                ...
                break
            case "model_car":// 汽车模型
                entityAttr.model = { uri: "../../Assets/model/car.gltf", scale: 0.2, minimumPixelSize: 50 }
                if (entityAttr.label) {
                    entityAttr.label.pixelOffset = new Cesium.Cartesian2(70, -30)
                } // 偏移量
                break
            case "model_air":// 飞机模型
                entityAttr.model = { uri: "../../Assets/model/plane.glb", scale: 0.1, minimumPixelSize: 50 }
                if (entityAttr.label) {
            ...
            case "model_weixin":// 卫星模型
                entityAttr.model = { uri: "../../Assets/model/satelight.gltf", scale: 1, minimumPixelSize: 100 }
                if (entityAttr.label) {
                    entityAttr.label.pixelOffset = new Cesium.Cartesian2(60, -30)
                } // 偏移量
                break
    ...
    getFlyOkPoints(position) {
        ...
        for (let i = 0, length = arr.length; i < length; i++) {
            var item = arr[i]
            if (item.time.dayNumber > thistime.dayNumber || item.time.secondsOfDay >
                thistime.secondsOfDay) {
                let len = Cesium.Cartesian3.distance(position, i === 0 ? item : arr[i - 1])
                alllen += len
                break
            }
            if (i > 0) {
    ...
    createTimeLine() {
        ...
        if (!viewer.animation) { // Animation
            ...
            const animation = new Cesium.Animation(animationContainer,
                new Cesium.AnimationViewModel(viewer.clockViewModel))
            this.animation = animation
        }
        if (!viewer.timeline) { // Timeline
    ...
    updateWall(positions) {
        ...
        if (!this.wallEntity) {
            const wallattr = WallAttr.style2Entity({ color: "#d7e600", outline: false, opacity: 0.5 })
            wallattr.minimumHeights = new Cesium.CallbackProperty(time => this._wall_minimumHeights, false)
            wallattr.maximumHeights = new Cesium.CallbackProperty(time => this._wall_maximumHeights, false)
            wallattr.positions = new Cesium.CallbackProperty(time => {
                const position = Cesium.Property.getValueOrUndefined(this.flyEntity.position, viewer.clock.currentTime, this.positionScratch)
    ...
    preRender_eventHandler(e) {
        ...
        if (Cesium.JulianDate.compare(viewer.clock.currentTime, viewer.clock.stopTime) >= 0) return this.onTimeEnd()
        ...
        const attr = this.flyEntity.data.properties.attr
        if (Cesium.JulianDate.compare(viewer.clock.currentTime, this.timesAtPoint[this.idxOfTimes]) >= 0) {
            const oldView = this.currentView
            this.currentView = this.flyEntity.position.getValue(this.timesAtPoint[this.idxOfTimes])
            this.currentOrientation = this.setOrientation(oldView, this.currentView)
            // if (this.idxOfTimes<this.timesAtPoint.length-1)
            ++this.idxOfTimes
        } else if (Cesium.JulianDate.compare(viewer.clock.currentTime, this.timesAtPoint[this.idxOfTimes - 1]) <= 0) {
//            this.currentView = this.flyEntity.position.getValue(this.timesAtPoint[this.idxOfTimes - 2])
        ...
        switch (attr.cameraType) {
            ...
            case "gs":// 锁定第一视角
                ...
                this.getModelMatrix(viewer.trackedEntity, viewer.clock.currentTime, this.scratch)


                viewer.scene.camera.lookAtTransform(this.scratch, new Cesium.Cartesian3(-attr.followedX, 0, attr.followedZ))
            ...
            case "sd":// 锁定上帝视角
                ...
                viewer.zoomTo(viewer.trackedEntity, new Cesium.HeadingPitchRange(this.currentOrientation.heading, -3.14 * 0.166667, transformZ * 2))
    ...
    getModelMatrix(entity, time, result) {
        ...
        var orientation = Cesium.Property.getValueOrUndefined(entity.orientation, time,
        ...
        if (!Cesium.defined(orientation)) {
            ...
            result = Cesium.Matrix4.fromRotationTranslation(
                Cesium.Matrix3.fromQuaternion(orientation, this.matrix3Scratch), position, result)
        }
        return result
    }
