...
function S3MTile(layer, parent, boundingVolume, fileName, rangeData, renderEntityMap, isLeafTile) {
    ...
    this.transform = layer.modelMatrix
...
S3MTile.prototype.visibility = function (frameState, parentVisibilityPlaneMask) {
    ...
    var clippingPlanes = layer.clippingPlanes;
    if (Cesium.defined(clippingPlanes) && clippingPlanes.enabled) {
        var intersection = clippingPlanes.computeIntersectionWithBoundingVolume(
            boundingVolume,
            layer.clippingPlanesOriginMatrix
        )
...
S3MTile.prototype.requestContent = function () {
    ...
    promise.then(function (arrayBuffer) {
        ...
        return contentFailedFunction(error)
    })
...
S3MTile.prototype.update = function (frameState, layer) {
    ...
    for (let key in renderEntityMap) {
        if (renderEntityMap.hasOwnProperty(key)) {
            // 为RenderEntity设置对应的ClippingPlaneCollection
            var tilesetClippingPlanes = layer.clippingPlanes
            renderEntityMap[key].referenceMatrix = layer.clippingPlanesOriginMatrix
            if (Cesium.defined(tilesetClippingPlanes) &&
                !Cesium.defined(renderEntityMap[key].clippingPlanes)) {
                renderEntityMap[key].clippingPlanes =
                    tilesetClippingPlanes.enabled && this._isClipped
            ...
            if (Cesium.defined(tilesetClippingPlanes) &&
                Cesium.defined(renderEntityMap[key].clippingPlanes) &&
                renderEntityMap[key].clippingPlanes !== tilesetClippingPlanes
            ) {
                renderEntityMap[key].clippingPlanes = tilesetClippingPlanes
            }

            // 为RenderEntity设置对应的FlattenPolygonCollection
            var tilesetFlattenPolygons = layer.flattenPolygons
            if (Cesium.defined(tilesetFlattenPolygons) &&
                !Cesium.defined(renderEntityMap[key].flattenPolygons)) {
                renderEntityMap[key].flattenPolygons =
                    tilesetFlattenPolygons.enabled
                        ? tilesetFlattenPolygons
                        : undefined
            }

            if (Cesium.defined(tilesetFlattenPolygons) &&
                Cesium.defined(renderEntityMap[key].flattenPolygons) &&
                renderEntityMap[key].flattenPolygons !== tilesetFlattenPolygons
            ) {
                renderEntityMap[key].flattenPolygons = tilesetFlattenPolygons
            }
