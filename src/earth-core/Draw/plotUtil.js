...
var plotUtil = (function (window) {
    ...
    var tailedAttackArrowDefualParam = {
        ...
        tailWidthFactor: .1,
        headTailFactor: .8,
        swallowTailFactor: 0//平尾和燕尾 ，0为平，1为燕。
    };

    var fineArrowDefualParam = {
        tailWidthFactor: 0.05,//0.05 0.15 前为平直箭头，后为宽尾箭头
    ...
    var xp = { version: "1.0.0", createTime: "2018.9.27", author: "muyao" };
    xp.fineArrowDefualParam = fineArrowDefualParam, xp.tailedAttackArrowDefualParam = tailedAttackArrowDefualParam, xp.doubleArrowDefualParam = doubleArrowDefualParam;
    xp.algorithm = {};
    xp.algorithm.doubleArrow = function (inputPoint) {
        ...
        xp.algorithm.tailedAttackArrow = function (inputPoint) {
            ...
            if (!(2 > t)) {
                ...
                var newArray = [];
                f.push(a), d = P.PlotUtils.getQBSplinePoints(d),
        ...
        xp.algorithm.fineArrow = function (tailPoint, headerPoint) {
            ...
            var height1 = tailPoint.length == 3 ? tailPoint[2] : 0;
            var height2 = headerPoint.length == 3 ? headerPoint[2] : 0;
            ...
            d.push(
                ...
                p[0], p[1], height2,
                u[0], u[1], height2,
                r[0], r[1], height2,//headerPoint
                c[0], c[1], height2,
                h[0], h[1], height2,
                l[0], l[1], height1,
                e[0], e[1], height1);//tailPoint
            return Cesium.Cartesian3.fromDegreesArrayHeights(d);
        },
        xp.algorithm.gatheringPlace = function (pnts) {
            //不支持高度
            pnts.forEach(e => e.length = 2)
            if (pnts.length < 2) {
                return [];
            } else {
                if (pnts.length === 2) {
                ...
                pList = pList.flatMap(e => { e.length = 2; return e })
                return Cesium.Cartesian3.fromDegreesArray(pList);
            }
        },
        xp.algorithm.closedCurve = function (pnts) {
            pnts.forEach(e => e.length = 2)
            if (pnts.length < 2) {
                return [];
            } else {
