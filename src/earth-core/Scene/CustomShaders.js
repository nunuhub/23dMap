 ...
 * @param {*} viewer
 */
 export function _getFlowLineShader (options) {
...
// 动态线
export function _getDynamicLineShader (options) {
  if (options && options.get) {
...
// 动态泛光线
export function _getDynamicLightLineShader (options) {
  if (options && options.get) {
    return "czm_material czm_getMaterial(czm_materialInput materialInput)\n\
                {\n\
                    czm_material material = czm_getDefaultMaterial(materialInput);\n\
 ...
 * @param {*} options
 */
export function _getDirectionWallShader_new (options) {
  ...
  \n\ ";
    if (options.freely == "vertical") { //（由下到上）
      ...
      materail += "vec4 colorImage = texture2D(image, vec2(fract(st.t), fract(st.s)));\n\ ";
    } else { //（逆时针）
...
export function _getDirectionWallShader (options) {
...
export function _getCircleFadeShader (options) {
...
// 波动圆
export function _getDynamicCircleShader (options) {
...
// 雷达扫描
export function _getRadarScanShader (options) {
...
// 圆形扫描
export function _getCircleScanShader (options) {
