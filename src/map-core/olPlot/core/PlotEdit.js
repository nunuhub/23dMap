...
class PlotEdit extends Observable {
  ...
  destroyHelperDom() {
    if (
      this.controlPoints &&
      Array.isArray(this.controlPoints) &&
      ...
      this.controlPoints.forEach((item, index) => {
        ...
        const element = htmlUtils.getElement(
          BASE_HELP_CONTROL_POINT_ID + '-' + index
        );
        if (element) {
          htmlUtils.off(
            element,
            'mousedown',
            this.controlPointMouseDownHandler.bind(this)
          );
          htmlUtils.off(
            element,
            'mousemove',
            this.controlPointMouseMoveHandler2.bind(this)
          );
        }
      });
      this.controlPoints = [];
