        ...
        if (source && source instanceof VectorSource) {
          ...
          features.forEach(feature => {
            if (
              feature &&
              feature.geometry &&
              feature.geometry.type !== 'PlotText'
            ) {
              if (
                feature.properties.type &&
                Geometry[feature.properties.type]
              ) {
                const feat = new Feature({
                  geometry: new Geometry[feature.properties.type](
                    [],
              ...
              _extents.push(
                new Point(feature.geometry.coordinates).getExtent()
              );
