/**
 * huht
 * 在autosize框架上进行二次开发，支持textarea宽度自适应
 */
const map =
  typeof Map === 'function'
    ? new Map()
...
function assign(ta) {
  if (!ta || !ta.nodeName || map.has(ta)) return;
  ...
  function resize() {
    const fontSize = computeFontSize(ta.innerHTML, ta.style);
    ...
    // / console.log(fontSize.height + heightOffset, ta.scrollHeight+heightOffset)
    ta.style.height = '';
    ta.style.height = fontSize.height + heightOffset + 6 + 'px';
    // ta.style.height = Math.max(30, fontSize.height + heightOffset + 6)+'px';

    ta.style.width = '';
    ta.style.width = fontSize.width + 10 + 'px';
    // ta.style.width = Math.max(100,fontSize.width + 10)+'px';
   ...
   * 计算字体大小
   */
  function computeFontSize(str, style) {
    // 对str进行分行\n
    ...
    if (strArray && strArray.length > 0) {
      for (const strSingle of strArray) {
        const size = computeSingleFontSize(strSingle, style);
        sizeD.width = Math.max(sizeD.width, size.width);
        sizeD.height = Math.max(sizeD.height, size.height);
        // console.log(sizeD.width, sizeD.height)
      }
      sizeD.height = sizeD.height * strArray.length;
    }
    return sizeD;
  }
   ...
   * 计算单行字体大小
   */
  function computeSingleFontSize(str, style) {
