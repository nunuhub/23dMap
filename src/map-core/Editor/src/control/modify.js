...
class ModifyControl extends Control {
  ...
  constructor(options) {
    ...
    Bus.$on('revokeFeatureEditing', () => {
      //this.deactivate(this.modifyInteraction);
      //this.map.refresh();
      //this.selectMove.getFeatures().clear();
      try {
        this.selectModify.getOverlay().getSource().clear(true);
      } catch (e) {
        this.selectModify.getOverlay().getSource().clear(true);
      }
    ...
    this.deleteNodeCondition = options.deleteNodeCondition || altKeyOnly;
    ...
    this.applySelectStyle = (feature, styleToApply) => {
      const featureStyles = getStyles(feature.getStyleFunction(), feature);
    ...
    this.onDeselectFeature = (feature, selectStyle, listenerPropName) => {
      ...
      const styles = getStyles(feature.getStyleFunction(), feature);
    ...
    this.selectCondition = (e) => {
      return singleClick(e) && !this.deleteNodeCondition(e);
    };

    this.selectModify = new Select({
      condition: options.modifyCondition || this.selectCondition,
    ...
    this.selectModify.getFeatures().on('add', (evt) => {
      /*setTimeout(() => {
        ...
        this.map.getView().setResolution(mapResolution + 0.00000000001);
      ...
      if (this.selectModifyStyle) {
        Bus.$emit('startFeatureEditing', {
          type: 'edit',
          select: this.selectModify,
          modify: this.modifyInteraction
        });
        // Apply the select style dynamically when the feature has its own style.
        this.onSelectFeature(
          evt.element,
    ...
    this.modifyInteraction = new Modify({
      ...
      deleteCondition: (e) => {
        if (this.deleteNodeCondition(e) && click(e)) {
          return true;
        } else {
          return false;
        }
    ...
    this.modifyInteraction.on('modifyend', () => {
      this.editor.setEditFeature(null);
      const mapres = this.map.getView().getResolution();
      this.map.getView().setResolution(mapres + 0.00000000000000001);
      this.map.getView().setResolution(mapres);
  ...
  setModifyStyle() {
    this.selectModifyStyle = new Style({
      image: new Circle({
        radius: 5,
        fill: new Fill({
          color: '#05A0FF'
  ...
  deactivate(silent) {
    ...
    //this.selectMove.getFeatures().clear();
    // let selectMoveFeatures = this.selectMove.getFeatures().getArray();
    // if (selectMoveFeatures.length > 0) {
    //   for (let i = selectMoveFeatures.length - 1; i >= 0; i--) {
    //     selectMoveFeatures = selectMoveFeatures.slice(i, 1)
    //   }
    // }
    // let selectModifyFeatures = this.selectModify.getFeatures().getArray();
    // if (selectModifyFeatures.length > 0) {
    //   for (let mi = selectModifyFeatures.length - 1; mi >= 0; mi--) {
    //     selectModifyFeatures = selectModifyFeatures.slice(mi, 1)
    //   }
    // }
    try {
      this.selectModify.getOverlay().getSource().clear(true);
