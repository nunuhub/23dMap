...
class Control extends OLControl {
  ...
  constructor(options) {
    ...
    if (!options.element) {
      ...
      button.style.height = options.height;
  ...
  activate(silent) {
    ...
    if (this.properties.activeImg) {
      this.img.src = this.properties.activeImg;
    }
    if (!silent) {
      this.dispatchEvent({
        type: 'change:active',
  ...
  deactivate(silent) {
    ...
    if (this.properties.deactiveImg) {
      this.img.src = this.properties.deactiveImg;
    }
    if (!silent) {
      this.dispatchEvent({
        type: 'change:active',
